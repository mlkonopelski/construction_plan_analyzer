...
1. **Faster R-CNN** - YOLO-family of models are using concept of anchor boxes to predict multiple bounding boxes per grid cell. Other approach is Region CNN which uses (in Fast and Faster version) features obtained on the whole image and Region proposal network deeper. It's slower but in this case I had high hopes that it gonna give me good results. Unfortunately I have a bug in very unobvious place and couldn't solve it in time. I used pre-trained (on COCO) Faster R-CNN Network from torchvision and during first forward propagation loss is calculated properly: `
-------INIT LOSS-------
{'loss_classifier': tensor(57.6800, grad_fn=<NllLossBackward0>), 'loss_box_reg': tensor(58.9167, grad_fn=<DivBackward0>), 'loss_mask': tensor(207.8854, grad_fn=<BinaryCrossEntropyWithLogitsBackward0>), 'loss_objectness': tensor(219.3661, grad_fn=<BinaryCrossEntropyWithLogitsBackward0>), 'loss_rpn_box_reg': tensor(45.0236, grad_fn=<DivBackward0>)}
-----------------------`
but after backpropagation of the first batch the whole loss is NaN since somewhere in weights I have NaN. `{'loss_classifier': tensor(nan, grad_fn=<NllLossBackward0>), 'loss_box_reg': tensor(nan, grad_fn=<DivBackward0>), 'loss_mask': tensor(nan, grad_fn=<BinaryCrossEntropyWithLogitsBackward0>), 'loss_objectness': tensor(nan, grad_fn=<BinaryCrossEntropyWithLogitsBackward0>), 'loss_rpn_box_reg': tensor(nan, grad_fn=<DivBackward0>)}` This isn't something I can easily fixed and therefore I cannot show results of this approach unfortuantely. Code is available in: `ml_models/torch_mask_rcnn/training.py`  
